...
const getFormBg = () => {
  ...
  // If it's a CSS variable, fallback to Brooklyn brown
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (typeof window !== 'undefined' && window.fathom) {
        window.fathom.trackEvent('Quote Submitted - QuoteForm');
      }
    } catch (error) {
            ...
            {showAddonsTray && (
              <div className="space-y-3">
                ...
                {formData.serviceType === 'moveout' && (
                  <div className="text-xs text-[#dfbd69] bg-[#dfbd69]/10 border border-[#dfbd69]/30 rounded-lg p-3">
                    <strong>Move Out:</strong> Includes all rooms, cabinets, baseboards, windows, and appliances
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {ADDONS.filter((addon) => {
                    ...
                    const isAutoIncluded =
                      (formData.serviceType === 'deep' &&
                        (addon.key === 'wallStainRemoval' || addon.key === 'baseboardCleaning' || addon.key === 'tileAndGrout')) ||
                      (formData.serviceType === 'moveout' &&
                        (addon.key === 'bedroomBathroomCabinets' || addon.key === 'insideKitchenCabinets' || addon.key === 'interiorWindows' || addon.key === 'insideFridge' || addon.key === 'insideOven' || addon.key === 'microwave' || addon.key === 'wallStainRemoval' || addon.key === 'tileAndGrout' || addon.key === 'baseboardCleaning'));
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-white/70 leading-relaxed">
                        We serve Brooklyn, Manhattan, Queens, Bronx, and Northern New Jersey (Jersey City, Hoboken, and surrounding areas). Contact us to confirm service in your area.
                      </p>
                    </div>
