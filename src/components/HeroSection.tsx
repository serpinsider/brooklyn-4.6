...
export default function HeroSection({ location = "Brooklyn, NY", title }: HeroSectionProps) {
  ...
  return (
    <section className={`${LAYOUTS.section.hero} ${COLORS.backgrounds.hero}`}>
      ...
      <div className={`relative z-10 mx-auto ${isFormExpanded ? 'w-full px-2' : 'max-w-7xl px-4 sm:px-6 lg:px-8'}`}>
        <div className={`relative gap-8 md:gap-12 xl:gap-20 ${
          isFormExpanded ? 'flex justify-center items-start' : 'grid grid-cols-1 md:grid-cols-2 items-center'
        }`}>
          {/* Hero Content - Hide when form is expanded */}
          <div className={`text-center md:text-left transition-all duration-500 ease-out max-w-xl ${
            isFormExpanded 
            ...
            <h1 className={TYPOGRAPHY.heroTitle}>
              {title 
                ...
                : location && location !== "Brooklyn, NY" 
                  ...
                  : "Book a Housekeeper in Brooklyn in 60 Seconds."
              }
            </h1>
            ...
            <div className="mt-8 flex items-center justify-center md:justify-start gap-3">
              ...
              <div className="flex">
                {avatars.map((avatar, index) => (
                  <Image
                    key={index}
                    src={avatar}
                    alt={`Brooklyn Maids customer ${index + 1}`}
                    width={32}
                    height={32}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="text-xs text-white drop-shadow-md whitespace-nowrap">
                  <span className="font-semibold">4.9/5</span> <span>(171 customers)</span>
                </div>
              </div>
            </div>
          </div>
          ...
          >
            <div 
              className="bg-white/20 backdrop-blur-md rounded-2xl shadow-xl border border-white/30 p-4 sm:p-6 relative w-full"
            >
