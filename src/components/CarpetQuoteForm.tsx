...
export default function CarpetQuoteForm() {
  ...
  const handleSubmit = async () => {
    ...
    const confirmNum = 'BK-CARPET-' + Math.random().toString(36).substring(2, 8).toUpperCase();
    ...
    try {
      const response = await fetch('https://formspree.io/f/mqazolgp', {
        ...
        body: JSON.stringify({
          ...
          _subject: `Brooklyn Maids - Carpet Cleaning Quote from ${formData.firstName} ${formData.lastName} - #${confirmNum}`,
