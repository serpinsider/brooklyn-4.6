...
export default function AirbnbQuoteForm() {
  ...
  const handleSubmit = async () => {
    ...
    const confirmNum = 'BK-AIRBNB-' + Math.random().toString(36).substring(2, 8).toUpperCase();
    const addons = [];
    ...
    try {
      const response = await fetch('https://formspree.io/f/mqazolgp', {
        ...
        body: JSON.stringify({
          ...
          _subject: `Brooklyn Maids - Airbnb Quote from ${formData.firstName} ${formData.lastName} - #${confirmNum}`,
