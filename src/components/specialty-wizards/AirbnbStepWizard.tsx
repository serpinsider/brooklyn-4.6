...
export default function AirbnbStepWizard({ onFormExpand }: AirbnbStepWizardProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError('');
    try {
      const confirmationNumber = 'BK-AIRBNB-' + Math.random().toString(36).substring(2, 8).toUpperCase();
      ...
      const response = await fetch('https://formspree.io/f/mqazolgp', {
        ...
        body: JSON.stringify({
          ...
          _subject: `Brooklyn Maids - Airbnb Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {currentStep === 4 && (
          <div className="mt-6 flex items-center justify-center gap-8">
            ...
            <div className="text-sm text-white/80"><span className="font-semibold">171</span> Happy Customers</div>
          </div>
        )}
      </div>
    </div>
