...
export default function CarCleaningStepWizard({ onFormExpand }: CarCleaningStepWizardProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const confirmationNumber = 'BK-CAR-' + Math.random().toString(36).substring(2, 8).toUpperCase();
      const response = await fetch('https://formspree.io/f/mqazolgp', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ...
          confirmationNumber,
          _subject: `Brooklyn Maids - Car Cleaning Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
