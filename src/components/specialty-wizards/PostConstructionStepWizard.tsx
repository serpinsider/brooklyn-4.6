...
export default function PostConstructionStepWizard({ onFormExpand }: PostConstructionStepWizardProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const confirmationNumber = 'BK-POST-' + Math.random().toString(36).substring(2, 8).toUpperCase();
      const response = await fetch('https://formspree.io/f/mqazolgp', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ...
          confirmationNumber,
          _subject: `Brooklyn Maids - Post-Construction Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
