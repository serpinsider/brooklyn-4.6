...
export default function HandymanStepWizard({ onFormExpand }: HandymanStepWizardProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const confirmationNumber = 'BK-HANDY-' + Math.random().toString(36).substring(2, 8).toUpperCase();
      const response = await fetch('https://formspree.io/f/mqazolgp', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ...
          confirmationNumber,
          _subject: `Brooklyn Maids - Handyman Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
          ...
          'Project Details': formData.jobDescription || 'No details provided',
        }) 
      });
