...
export default function CommercialStepWizard({ onFormExpand }: CommercialStepWizardProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const confirmationNumber = 'BK-COMM-' + Math.random().toString(36).substring(2, 8).toUpperCase();
      const response = await fetch('https://formspree.io/f/mqazolgp', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ...
          confirmationNumber,
          _subject: `Brooklyn Maids - Commercial Cleaning Quote from ${formData.firstName} ${formData.lastName} - #${confirmationNumber}`,
